...
// ... [License comments remain unchanged] ...
...
impl Controller {
    ...
    pub fn fas_update_freq(&mut self, control: isize) {
        ...
        #[cfg(debug_assertions)]
        debug!("policy{} freq: {}", self.cpu_infos.last().unwrap().policy, fas_freq_max);
    
        let _ = self.cpu_infos.last_mut().unwrap().write_freq(fas_freq_max, &mut self.file_handler);
        ...
        for cpu in self.cpu_infos.iter_mut().rev().skip(1) {
            ...
            if cpu_usage < 10.0 {
                ...
                OFFSET_MAP.get().unwrap().get(&cpu.policy).unwrap().store(new_offset, Ordering::Release);
            } else if cpu_usage > 80.0 {
                let new_offset = offset.saturating_add(2);
                OFFSET_MAP.get().unwrap().get(&cpu.policy).unwrap().store(new_offset, Ordering::Release);
            }
